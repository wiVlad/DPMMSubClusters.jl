...
function init_model()
    ...
    labels =
        distribute(rand(1:initial_clusters, (size(data, 2))) .+ ((outlier_mod > 0) ? 1 : 0))
...
function init_first_clusters!(dp_model::dp_parallel_sampling, initial_cluster_count::Int64)
    if outlier_mod > 0
        push!(
            dp_model.group.local_clusters,
            create_outlier_local_cluster(dp_model.group, outlier_hyper_params),
        )
    end
    ...
    broadcast_cluster_params(
        [create_thin_cluster_params(x) for x in dp_model.group.local_clusters],
        [1.0],
    )
end
...
function fit(
    ...
    [[
        x.cluster_params.cluster_params.distribution.Σ
        for x in dp_model.group.local_clusters
    ]]
end
...
function fit(
    ...
    return Array(dp_model.group.labels),
    [x.cluster_params.cluster_params.distribution for x in dp_model.group.local_clusters],
    dp_model.group.weights,
    iter_count,
    nmi_score_history,
    liklihood_history,
...
fit(
    ...
    Float32(α_param),
    iters = Int64(iters),
    init_clusters = Int64(init_clusters),
    seed = seed,
    verbose = verbose,
...
function run_model(dp_model, first_iter, model_params = "none", prev_time = 0)
    ...
    for i = first_iter:iterations
        ...
        if i >= iterations - split_stop ||
           length(dp_model.group.local_clusters) >= max_num_of_clusters
            no_more_splits = true
        end
        ...
        if ground_truth != nothing
            ...
            push!(
                nmi_score_history,
                mutualinfo(Int.(ground_truth), group_labels, normed = true),
            )
        else
        ...
        if use_verbose
            ...
            println(
                ...
                " || Log posterior: " *
                string(liklihood_history[end]) *
                " || Vi score: " *
                string(v_score_history[end]) *
                " || NMI score: " *
                string(nmi_score_history[end]) *
                " || Iter Time:" *
                string(iter_time) *
                " || Total time:" *
                string(sum(iter_count)),
            )
        else
...
function calculate_posterior(model::dp_parallel_sampling)
    log_posterior =
        logabsgamma(model.model_hyperparams.α)[1] -
        logabsgamma(size(model.group.points, 2) + model.model_hyperparams.α)[1]
    for cluster in model.group.local_clusters
        if cluster.cluster_params.cluster_params.suff_statistics.N == 0
            continue
        end
        log_posterior += log_marginal_likelihood(
            cluster.cluster_params.cluster_params.hyperparams,
            cluster.cluster_params.cluster_params.posterior_hyperparams,
            cluster.cluster_params.cluster_params.suff_statistics,
        )
        log_posterior +=
            log(model.model_hyperparams.α) +
            logabsgamma(cluster.cluster_params.cluster_params.suff_statistics.N)[1]
    end
